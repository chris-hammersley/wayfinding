...
class Water extends Mesh {
	...
	constructor( geometry, options = {} ) {
		...
		const color = ( options.color !== undefined ) ? new Color( options.color ) : new Color( 0x728295 );
		const textureWidth = options.textureWidth !== undefined ? options.textureWidth : 2048;
		const textureHeight = options.textureHeight !== undefined ? options.textureHeight : 2048;
		...
		const scale = options.scale !== undefined ? options.scale : 10;
		const shader = options.shader !== undefined ? options.shader : Water.WaterShader;

		const textureLoader = new TextureLoader();

		const flowMap = options.flowMap || undefined;
		const normalMap0 = options.normalMap0 || textureLoader.load( '/assets/textures/water/Water_1_M_Normal.jpg' );
		const normalMap1 = options.normalMap1 || textureLoader.load( '/assets/textures/water/Water_2_M_Normal.jpg' );
